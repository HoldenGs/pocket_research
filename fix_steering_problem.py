...
class SteeringAwareDataset(Dataset):
...
def train_steering_aware_model():
    ...
    print("🎯 TRAINING STEERING-AWARE MODEL")
    print("=" * 50)
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), 'models/steering_fixed_model.pth')
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load('models/steering_fixed_model.pth'))
    ...
    plt.tight_layout()
    plt.savefig('training_curve_steering_fixed.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"📈 Training curves saved as: training_curve_steering_fixed.png")
    
    print(f"\n🎯 EVALUATING STEERING-FIXED MODEL")
    print("=" * 40)
    ...
    ax1.set_title('Steering Error Distribution (Steering-Fixed Model)', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('Throttle Error Distribution (Steering-Fixed Model)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('error_distributions_steering_fixed.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    print(f"\n📈 Error distribution plots saved as: error_distributions_steering_fixed.png")
    print(f"🏁 Model saved as: models/steering_fixed_model.pth")
